/*
 * Shared and maintained by [Nezasa](http://www.com)
...
iso8601.Period.parseToString = function (period, unitNames, unitNamesPlural, distributeOverflow) {
  ...
  for (let i = 0; i < durationPerUnit.length; i++) {
    if (durationPerUnit[i] > 0) {
      if (durationPerUnit[i] === 1) {
        result[i] = durationPerUnit[i] + ' ' + unitNames[i]
      } else {
        result[i] = durationPerUnit[i] + ' ' + unitNamesPlural[i]
      }
    }
  }
 ...
 */
function parsePeriodString (period, distributeOverflow = false) {
  ...
  var struct
  var reg = /^P((\d+Y)?(\d+M)?(\d+W)?(\d+D)?)?(T(\d+H)?(\d+M)?(\d+S)?)?$/
  ...
  if (!period) return duration
    ...
    return new Error('Invalid iso8601 period string \'' + period + '\'')
  }

  // parse the string
  if (reg.exec(period)) {
    struct = reg.exec(period)
    ...
    return Error("String '" + period + "' is not a valid ISO8601 period.")
  }
